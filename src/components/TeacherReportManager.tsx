...
export const TeacherReportManager: React.FC<TeacherReportManagerProps> = ({
  ...
  const handleSingleAddStudent = (e: React.FormEvent) => {
    ...
    if (onAddStudent) {
      onAddStudent(newStudentName.trim(), selectedClass, newStudentNim.trim() || undefined);
    }
    setNewStudentName('');
    setNewStudentNim('');
    setSuccessNotice(`✅ Berhasil menambahkan murid "${newStudentName}" di Kelas ${selectedClass}!`);
    setTimeout(() => setSuccessNotice(''), 3000);
  };

  const handleBatchAddStudents = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    const newEntries: Student[] = lines.map((line, idx) => {
      ...
      return {
        id: `std-${selectedClass.toLowerCase()}-${Date.now()}-${idx}`,
        classGroup: selectedClass,
        name,
        nim,
        attendanceNo: classStudents.length + idx + 1,
      };
    });

    if (onSaveClassRoster) {
      onSaveClassRoster(selectedClass, [...classStudents, ...newEntries]);
    } else if (onAddStudent) {
      newEntries.forEach((st) => onAddStudent(st.name, selectedClass, st.nim));
    }

    setBatchNamesText('');
    setSuccessNotice(`✅ Berhasil menambahkan ${newEntries.length} murid baru ke Kelas ${selectedClass}!`);
    setTimeout(() => setSuccessNotice(''), 3000);
  };
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-3 sm:p-6 shadow-xl border-4 border-amber-400 space-y-6 max-w-full overflow-hidden">
      {/* Top Banner */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-4 border-b-2 border-dashed border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-amber-400 rounded-2xl flex items-center justify-center text-blue-950 font-black shadow-md shrink-0">
            <Award className="w-6 h-6 sm:w-7 sm:h-7 text-blue-950" />
          </div>
          <div className="min-w-0">
            <span className="text-[10px] font-black uppercase bg-amber-400 text-blue-950 px-2.5 py-0.5 rounded-full shadow-sm inline-block">
              MODE GURU & PIHAK SEKOLAH
            </span>
            <h2 className="text-base sm:text-xl font-black text-blue-950 dark:text-white uppercase mt-0.5 break-words">
      ...
      {/* Select Student Selector & Add Student Bar */}
      <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border-2 border-slate-200 dark:border-slate-700 space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-slate-700">
          <label className="text-xs font-black text-blue-950 dark:text-slate-200 uppercase flex items-center gap-1.5">
            ...
            <span>Pilih Murid Kelas {selectedClass} Untuk Diisi Laporannya ({classStudents.length} Siswa):</span>
          </label>

          <button
            type="button"
            onClick={() => setIsManageRosterOpen(!isManageRosterOpen)}
            className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs px-3.5 py-2 rounded-xl border-2 border-blue-500 shadow-md transition-all uppercase"
          >
            <UserPlus className="w-4 h-4 text-yellow-300" />
            <span>{isManageRosterOpen ? 'Tutup Panel Tambah Murid' : '➕ TAMBAH NAMA MURID KELAS'}</span>
          </button>
        </div>
        ...
        {classStudents.length > 0 ? (
          <select
            ...
            className="w-full bg-white dark:bg-slate-900 text-blue-950 dark:text-white font-black text-sm p-3 rounded-xl border-2 border-slate-300 dark:border-slate-700 outline-none shadow-sm"
          >
            {classStudents.map((s) => (
          ...
          <p className="text-xs font-bold text-amber-600 bg-amber-50 dark:bg-amber-950/40 p-3 rounded-xl border border-amber-300">
            ⚠️ Belum ada murid terdaftar di Kelas {selectedClass}. Silakan klik tombol "➕ TAMBAH NAMA MURID KELAS" di atas untuk memasukkan nama siswa!
          </p>
        )}

        {/* Expandable Add Student / Manage Roster Box */}
        {isManageRosterOpen && (
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border-2 border-blue-300 dark:border-blue-800 space-y-5 animate-fade-in">
            <div className="flex items-center justify-between border-b pb-2 dark:border-slate-800">
              <h4 className="text-xs font-black text-blue-950 dark:text-yellow-400 uppercase flex items-center gap-2">
                <UserPlus className="w-4 h-4 text-blue-600 dark:text-yellow-400" />
                <span>KELOLA ROSTER NAMA SISWA (KELAS {selectedClass})</span>
              </h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Option A: Tambah Satu Murid */}
              <form onSubmit={handleSingleAddStudent} className="space-y-3 bg-blue-50/50 dark:bg-slate-800/50 p-3.5 rounded-xl border border-blue-200 dark:border-slate-700">
                <h5 className="text-[11px] font-black text-blue-900 dark:text-blue-300 uppercase">
                  A. Tambah Satu Murid:
                </h5>
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs font-bold p-2 rounded-lg outline-none"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black text-slate-700 dark:text-slate-300 block mb-1">
                ...
                <button
                  type="submit"
                  className="w-full bg-green-600 hover:bg-green-700 text-white font-black text-xs py-2 px-3 rounded-lg border border-green-500 uppercase flex items-center justify-center gap-1.5 transition-all shadow-sm"
                >
              ...
              {/* Option B: Input Banyak Nama Sekaligus (Batch) */}
              <form onSubmit={handleBatchAddStudents} className="space-y-3 bg-amber-50/50 dark:bg-slate-800/50 p-3.5 rounded-xl border border-amber-200 dark:border-slate-700">
                <h5 className="text-[11px] font-black text-amber-900 dark:text-amber-300 uppercase">
                  B. Tempel Banyak Nama Sekaligus (Per Baris):
                </h5>
                <textarea
                  rows={4}
                  ...
                  className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs font-mono p-2 rounded-lg outline-none"
                />
                <button
                  type="submit"
                  className="w-full bg-amber-500 hover:bg-amber-600 text-blue-950 font-black text-xs py-2 px-3 rounded-lg border border-amber-400 uppercase flex items-center justify-center gap-1.5 transition-all shadow-sm"
                >
            ...
            {/* Current Students List in Selected Class */}
            {classStudents.length > 0 && (
              <div className="space-y-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                <h5 className="text-[11px] font-black text-slate-700 dark:text-slate-300 uppercase">
                  Daftar Murid Terdaftar Saat Ini ({selectedClass}):
                </h5>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-48 overflow-y-auto p-1">
                  {classStudents.map((st, idx) => (
                    <div
                      key={st.id}
                      className="flex items-center justify-between bg-slate-100 dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700 text-xs font-bold"
                    >
                      <span className="truncate pr-1">
                        <span className="text-amber-600 font-black mr-1">{idx + 1}.</span>
                        {st.name} {st.nim ? <span className="text-[10px] text-slate-500">({st.nim})</span> : ''}
                      </span>
                      {onDeleteStudent && (
                        <button
                          type="button"
                          onClick={() => {
                            if (confirm(`Hapus murid "${st.name}" dari Kelas ${selectedClass}?`)) {
                              onDeleteStudent(st.id);
                            }
                          }}
                          className="text-rose-500 hover:text-rose-700 p-1 rounded hover:bg-rose-100 dark:hover:bg-rose-950/50 shrink-0"
                          title="Hapus Murid"
                        >
